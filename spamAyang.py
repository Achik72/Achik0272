...
def logo_taekyung():
    try:
        os.system("xdg-open https://www.instagram.com/achik0272/")
        time.sleep(3)
        os.system("clear")
        print(f"""
...
\033[1;92m
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
<<<<<<>>>>BY ACHIK0272 FOR AYANG<<<<<>>>
 ____    ____  ___    ___  ____    ____  ___   ___
|    \  /    ||   |  |   ||    \  /    ||  |  |  |
|     \/     ||   |  |   ||     \/     ||  |  |  |
|  |\    /|  ||   |__|   ||  |\    /|  ||  |__|  |
|__| \__/ |__||__________||__| \__/ |__||________|
\033[1;91m\033[1;41m\033[1;97m Author: By ACHIK72 \033[;0m\033[1;91m\033[1;92m
""")
...
def put():
    ...
    try:
        ...
        mengetik("\033[1;30m<════════════[\033[1;33;41m • \033[1;37m MASUKIN Nomor Ayang \033[1;33m• \033[0m\033[1;30m]══════════════>")
        mengetik("\033[37m[\033[31m•\033[37m]\033[32m  nomor ayang \033[37m : \033[37m\033[33m881025044067\033[33m")
        mr_am=input(f"\n\033[37m[\033[31m•\033[37m]\033[32m Nomor HP  Ayang\033[32m \033[37m:\033[37m\033[33m ")
        mengetik("\033[1;30m<════════════[\033[1;33;41m • \033[1;37m BEBAS JUMLAH SEKANGENNYA  \033[1;33m• \033[0m\033[1;30m]═════════════>")
        ...
        for i in range(int(power_python)):
            ...
            if "call" in jag:
                mengetik(f"{W}[{G}✓{W}] Tunggu ya ayang {Y}{mr_am}")
            else:
                mengetik(f"{W}[{R}•{W}] DI PROSES AYANG KIRIM SMS CINTA KE  {Y}{mr_am}")
            time.sleep(1)
            ...
            gas = sms.text
            if "Berhasil ayang" in gas:
                mengetik(f"{W}[{R}×{W}] Berhasil ayang,Tunggu ya {Y}{mr_am}")
            else:
                mengetik(f"{W}[{G}✓{W}] Berhasil ayang, sms cintanya masuk  {Y}{mr_am}")
            time.sleep(1)
            ...
            if "ok" in dekor2:
                mengetik(f"{W}[{G}✓{W}] Berhasil Ayang,Sabar ya {Y}{mr_am}")
            else:
                mengetik(f"{W}[{R}×{W}] Gagal ayang,coba lagi ya {Y}{mr_am}")
            time.sleep(1)
            ...
            if "neng" in buka:
                mengetik(f"{W}[{R}×{W}] Gagal deh,jangan ngambek ya {Y}{mr_am}")
            else:
                mengetik(f"{W}[{G}✓{W}] bentar lagi deh,lagi merangkai kata nih  {Y}{mr_am}")
            time.sleep(1)
            ...
            if "erors" in gaskeun:
                mengetik(f"{W}[{R}×{W}] ah ga bisa nih, kamu kurang kangen kayanya  {Y}{mr_am}")
            else:
                mengetik(f"{W}[{G}✓{W}] selesai ,untuk keluar ketik ctrl + c ya ayang  {Y}{mr_am}")
            time.sleep(1)
        ...
        sys.exit(f"{W}[{R}!{W}] Eror Cek Jaringan ayang dulu deh{abu}....{abu}")
    except KeyboardInterrupt:
        sys.exit(f"{W}[{R}!{W}] Keluar ya,nanti spam lagi ya ayang,aku tunggu{abu}...{W}")
    except ValueError:
        sys.exit(f"{W}[{Y}!{W}] jangan coba coba masukin nomor orang lain deh{abu}.....")
